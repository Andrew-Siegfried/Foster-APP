...
<button class="button">
    <div class = "uk-width-1-1 uk-grid " >
        <div class = "uk-margin-small-top uk-width-2-5">
            <h4><img style="width:32px;height:32px;text-align:left;" src="../assets/images/invoice-icon.svg" alt=""> {{object.month}} {{object.year}} Invoice - ({{fp_info.records[0].fname}} {{fp_info.records[0].lname}} ID:{{fp_info.records[0].id}})</h4>
        </div>
...
export default {
  ...
  created() {
    ...
    axios.get('http://www.fizzlab.xyz:86/api/api.php/records/users?filter=user_id,eq,'+this.object.fp_id)
    .then(res => {
      this.fp = res.data; 
