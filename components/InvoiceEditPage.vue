...
export default {
    ...
    props: ["user","child_info","invoice_info","object"],
    components: {
        Datepicker
    },
    methods: {
        editInvoice(invoice_info) {
            ...
            let len = this.invoice_info.length;
            let i = 0;
            console.log("len "+len);
            for (i = 0; i < len; i++) 
            {
                axios.put(
                    "http://www.fizzlab.xyz:86/api/api.php/records/invoice_information/"+this.invoice_info[i].id,
                    {
                        "date_from": this.invoice_info[i].date_from,
                        "date_to": this.invoice_info[i].date_to,
                        "units": this.invoice_info[i].units,
                        "rates": this.invoice_info[i].rates
                    },
